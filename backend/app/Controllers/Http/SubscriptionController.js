...
class SubscriptionController {
  ...
  async store({ response, params, auth }) {
    ...
    const checkSubscription = await user.subscriptions().where('section', workshop.section).first()

    if (checkSubscription) {
      return response.status(400).json({ error: 'You cannot subscribe to two workshops in the same section.' })
    }
