...
test('it should be able to create workshops', async ({
  assert,
  client,
}) => {
  ...
  const response = await client
    ...
    .send({
      ...
      description: 'Se você já buscou sobre as melhores linguagens back-end atuais provavelmente deve ter se deparado com o NodeJS. Nesse post vamos abordar sobre as reais utilidades do Node comparando com outras linguagens conhecidas como PHP, Ruby, Python, Elixir, etc...',
      user_id: user.id,
      section: 1
    })
    .end()
...
test('it should be able to list workshops', async ({
  assert,
  client,
}) => {
  const user = await Factory.model('App/Models/User').create()
...
test('it should be able to update a workshop', async ({
  assert,
  client,
}) => {
  const user = await Factory.model('App/Models/User').create()
...
test('it should be able to delete a workshop', async ({
  assert,
  client,
}) => {
  const user = await Factory.model('App/Models/User').create()
